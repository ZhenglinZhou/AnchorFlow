...
function applyGeometryModeTo(modelViewer) {
    if (!modelViewer || !modelViewer.model || !modelViewer.model.materials || modelViewer.model.materials.length === 0) {
        return;
    }

    const firstMat = modelViewer.model.materials[0];
    const texSlot = firstMat.pbrMetallicRoughness.baseColorTexture;
    // if (!texSlot || texSlot.texture === null) {
    //     // 已经是几何模式了，直接返回
    //     return;
    // }
...
// 原始按钮用的 hideTexture 继续保留，只是内部复用上面的逻辑
function hideTexture() {
    let appearanceButton = document.getElementById('appearance-button');
    let geometryButton = document.getElementById('geometry-button');
    if (appearanceButton && geometryButton) {
        appearanceButton.classList.remove('checked');
        geometryButton.classList.add('checked');
    }
    let modelViewer = document.getElementById('modelviewer');
    applyGeometryModeTo(modelViewer);
}
